...
int main(){
	...
	double q,hbarc=197.326,mass=493.677;
	int i;
	...
	//CWaveFunction_generic *wf=new CWaveFunction_pipluspiplus_sqwell("parameters/kparameters.dat");
	CWaveFunction_generic *wf=new CWaveFunction_generic("parameters/kparameters.dat",0.0,mass,mass,1.0);
	...
	//S2CF::s2c_gauss(scalc,kernelwf,c3d);
	//S2CF::s2c(s3d,wf,c3d);
	printf("_____________ c3d _________________\n");
	c3d->PrintProjections();
	sprintf(dirname,"%s/cfdata/kkbosons_gauss_Rx%g_Ry%g_Rz%g_lambda%g/3d",dataroot,Rx,Ry,Rz,lambda);
	//sprintf(dirname,"%s/cfdata/kk_gauss_Rx%g_Ry%g_Rz%g_lambda%g/3d",dataroot,Rx,Ry,Rz,lambda);
