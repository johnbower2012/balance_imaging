...
int main(int argc, char* argv[]){
  ...
  *************************************/
  ...
  for(int i=0;i<parameters;i++)
    {
      WidthScaled(i) = (rangeScaled(i,1) - rangeScaled(i,0))/40.0;
    }
  ...
  linearRegressionLeastSquares(ModelZ,paramScaled,Beta);
  WriteFile("beta.dat",Beta," ");
  int row=8;
  ExpZ = ModelZ.row(row);
  std::cout << row << " row: " << paramScaled.row(row) << " " << ExpZ << std::endl;
  emulator emulation(paramScaled, Hyperparameters, Beta);
  //ExpZ = ModelZ.row(4);
  MCMC mcmc(ExpZ,rangeScaled,WidthScaled,true);
  mcmc.setPosition();
  int Samples=10000;
  ...
  WriteCSVFile("mcmctrace.csv",paramNames,ntrace,",");
  ...
  *************************************/
  ...
  for(int row=0;row<rows;row++){
    ...
    Eigen::MatrixXd testY_ = ModelZ.block(row,0,1,observables),
      //PM = Parameters,
      PM = paramScaled,
      MZ = ModelZ.block(0,0,ModelZ.rows(),observables);
  ...
  WriteFile("fit.dat",print,delimiter);
}
