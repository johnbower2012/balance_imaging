...
int main(int argc, char* argv[]){
  ...
  int action=1; //1 = mcmc, 2 = test emulator

  /*************************************
   *    Write Parameters files        *
  *************************************/
  std::string 
    foldername="model_output",                       //foldername = model BF location
    writefilename="parameters.dat",                  //writefilename = paramfile name, should default to parameters.dat
    rangename=foldername+"/parameter_priors.dat",    //rangename = rangefile for parameters and paramnames to LCHsample for model run
    infilename=foldername+"/moments_parameters.dat", //output parameters used for model runs
    delimiter=" ",                                   //delimiter for writing and reading files
    gabname=foldername+"/gabfunctions.dat";          //write prior gab functions, in order eta, uu0, ud0, us0, ss0, uu1, ud1, ...
  ...
  *************************************/
  ...
  ConductModelAnalysis(foldername, delimiter, start, finish, ModelZ, ExpZ);
  ...
  *************************************/
  ...
  WriteFile(foldername+"/hyperparameters.dat",Hyperparameters,delimiter);
  //Load Range File
  ...
  WriteFile(foldername+"/scaledbeta.dat",Beta," ");
  /*
  int row=5;
  ExpZ = ModelZ.row(row);
