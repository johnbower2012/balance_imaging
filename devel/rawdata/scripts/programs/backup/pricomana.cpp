...
arma::mat calculate_zmatrix_function(arma::mat val_matrix, arma::vec mean_vec, arma::vec obs_error){
	int i, j,
		reps = val_matrix.n_rows,
		dims = val_matrix.n_cols;
	double xi_mean, xi_error;
	arma::mat zval_matrix = arma::zeros<arma::mat> (reps,dims);

	for(i=0;i<dims;i++){
		xi_mean = mean_vec(i);
		xi_error = obs_error(i);
		for(j=0;j<reps;j++){
		  zval_matrix(j,i) = (val_matrix(j,i) - xi_mean)/xi_error;
		}
	}
	return zval_matrix;
}
...
arma::mat sort_eigenvectors_function(arma::vec& eigval_vec, arma::mat& eigvec_matrix){
	...
	for(i=0;i<dims;i++){
		...
		eigval_vec(i) = eigval_matrix(i,0);
	}
...
//full transformation from val_matrix to tval_matrix USING UNIT VARIANCE, return mean & cov matrices
...
//full transformation from val_matrix to tval_matrix USING ERROR, return mean
arma::mat calculate_tmatrix_function(arma::mat val_matrix, arma::vec obs_error, arma::vec& eigval_vec, arma::mat& eigvec_matrix, arma::mat& mean_vec, arma::mat& zcov_matrix){
	int reps = val_matrix.n_rows,
		dims = val_matrix.n_cols;

	arma::mat	zval_matrix = arma::zeros<arma::mat>(reps,dims),
				tval_matrix = zval_matrix,
	  eigsort_matrix = zcov_matrix;

	zcov_matrix = arma::zeros<arma::mat>(dims,dims);
	eigvec_matrix = arma::zeros<arma::mat>(dims,dims);
	mean_vec = arma::zeros<arma::vec>(dims);

	mean_vec = calculate_mean_function(val_matrix);
	zval_matrix = calculate_zmatrix_function(val_matrix, mean_vec, obs_error);
	zcov_matrix = calculate_zcovariance_function(zval_matrix);
...
void writeFile(std::string filename, arma::vec val_vec){
  std::ofstream ofile;
  int reps = val_vec.n_elem;

  ofile.open(filename);
  for(int i=0;i<reps;i++){
    ofile << val_vec(i) << std::endl;
  }
  ofile.close();
}
