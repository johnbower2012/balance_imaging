...
void CAcceptance_STAR::star_acc_eff(int pid,double pt,double eta,double phi,int cen,bool &accept,double &eff){
	...
	//   |eta| < 1.0
	//   full phi acceptance
	//   calculate the efficiency based on particle ID, pt, eta, and centrality
	
	// This routine is set up for full STAR TPC+TOF
	// Modified by GDW January, 2013
	//  modifed upper cut to be p instead of pt
	//  fixed an oopsie in the pions
	...
	if((cen < 0) || (cen > 4)){
		printf("centrality index makes no sense, =%d\n",cen);
		exit(1);
		return;
	}
	if(pt < 0.2){
	...
	eff*=0.95;  // added by Scott, two-sigma cut for TOF in PID from Hui's thesis
	//printf("pt=%g, eff=%g\n",pt,eff);
}
