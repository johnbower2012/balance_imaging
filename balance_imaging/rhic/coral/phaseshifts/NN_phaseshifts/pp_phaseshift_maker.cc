...
int main(){
  ...
  double qsmooth,q,tlab,elab,plab,roots,delq;
...
double schroedinger(double q){
  ...
  while(phase0<PI) phase0+=2.0*PI;
  while(phase0>PI) phase0-=2.0*PI;
  phase00=phase0;
  //printf("q=%g: With No Reid, phase00=%g, sigma=%g\n",q,(phase00*180.0/PI)+90.0,
  // sigma*180.0/PI);
  ...
  while(delta>PI) delta-=PI;
  //sigma=sigma-(fabs(sigma)/sigma)*2.0*PI*floor(fabs(sigma)/(2.0*PI));
  //printf("___ q=%g, sigma=%g, sigma-phase0-0.5*PI=%g, delta=%g ___\n",
  // q,sigma,sigma-phase0-0.5*PI,delta);
  if(fabs(q+1.0E-8-int(q+1.0E-8))<1.0E-5)
    printf("___ q=%g, delta=%g ___\n",q,(180.0/PI)*delta);
  /*
    for(ir=1;ir<6000;ir+=1000){
    r1=ir*schr_delr;
    psi1=psi[ir]+exp(-2.0*ci*(delta-sigma))*conj(psi[ir]);
    printf("r=%g,  delta=%g, psi=(%g,%g), psi1=(%g,%g), |psi|^2=%g\n",r1,delta,
    real(psi[ir]),imag(psi[ir]),real(psi1),imag(psi1),
    real(psi1*conj(psi1)));
    }*/
  delete [] psi;
  return delta;
