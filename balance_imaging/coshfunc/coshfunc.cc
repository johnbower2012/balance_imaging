...
int main(int argc,char *argv[]){
	...
	double ddeta=0.00001,g0,g;
	for(n=0;n<nmax;n++){
		g0=coshexpand.GetG(n,0.0);
		printf("----- n=%d ------\n",n);
		for(eta=0.0;eta<2.5*ddeta;eta+=ddeta){
			g=coshexpand.GetG(n,eta);
			printf("%6.4f %g %12.10f\n",eta,g-g0,((g-g0)/(eta*eta))/(g0*(n+1)*(n+1)));
		}
	}
	printf("---------------\n");
	for(eta=0;eta<10;eta+=0.05){
		printf("%6.3f ",eta);
		for(n=0;n<nmax;n++){
			printf("%8.5f ",coshexpand.GetG(n,eta));
		}
		printf("\n");
	}
	...
	for(eta=0.5*deta;eta<8;eta+=deta){
		...
		df=dumbfunc(eta);
		printf("%4.3f %g =? %g, ratio=%g\n",eta,dumbo,df,dumbo/df);
		fprintf(fptr,"%4.3f %9.6f %9.6f, ratio=%g\n",eta,dumbo,df,dumbo/df);
	}
	fclose(fptr);
