...
CRandom::CRandom(int seed){
	generator=new boost::random::mt19937(seed);
}
...
double CRandom::ran(void){
  return random_uniform(*generator);
}

void CRandom::gauss2(double *g1,double *g2){
  double x,y,r2,r,c,s;
TRY_AGAIN:
  x=1.0-2.0*random_uniform(*generator);
  y=1.0-2.0*random_uniform(*generator);
  r2=x*x+y*y;
...
long unsigned int CRandom::iran(unsigned long int imax){
  return lrint(floor(imax*random_uniform(*generator)));
}


double CRandom::gauss(void){
  return random_normal(*generator);
}
...
int CRandom::GetNPoissonian(double eta){
	double x;
	boost::poisson_distribution<int> random_poisson(eta);
  return random_poisson(*generator);
}
